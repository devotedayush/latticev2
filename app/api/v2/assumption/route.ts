...
import type { AssumptionState } from "@/lib/v2";
import { updateAssumptionState } from "@/lib/v2-db";

const allowed: AssumptionState[] = ["holds", "at_risk", "invalidated", "reconfirmed"];
...
export async function PATCH(request: Request) {
  ...
  let body: { id?: string; state?: AssumptionState; teamId?: string };
  try {
  ...
  try {
    ...
    const state = await updateAssumptionState(auth.supabase, body.id, body.state, team.id);
    return NextResponse.json({ state, team });
  } catch (err) {
    console.error("/api/v2/assumption PATCH failed", err);
